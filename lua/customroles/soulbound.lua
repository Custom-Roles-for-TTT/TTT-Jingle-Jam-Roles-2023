...
if SERVER then
    ...
    net.Receive("TTT_SoulboundUseAbility", function(len, ply)
        ...
        if not ply:IsSoulbound() and ability.SoulboundOnly then return end
        ...
        for _, p in PlayerIterator() do
            if p:IsSoulbound() or p.TTTIsGhosting then
                ...
                for i = 1, max do
                    ...
                    if not p:IsSoulbound() and ability.SoulboundOnly then return end
...
if CLIENT then
    ...
    local function OpenSoulboundShop(isSoulbound)
        ...
        local function FillAbilityList(abilities, isSoulbound)
            dlist:Clear()
            ...
            for _, ability in pairs(abilities) do
                if not ability:Enabled() then continue end
                if not isSoulbound and ability.SoulboundOnly then continue end
